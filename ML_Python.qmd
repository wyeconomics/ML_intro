...
### Tree-based Models(Decision Tree and Random Forests){#sec-tree}
...
In contrast to `KNeighborsClassifier`, `DecisionTreeClassifier` does not require one-hot encoding and standardizing. However, it does require each string variable to be converted to integers using `label encoding`.
...
is a function of $\xi^{tr}$ and $\alpha$. In contrast to @eq-lasso_beta, I added the subscript $\alpha$, because this is not our final choice of $\hat{\beta}$; rather, it is the interim value of $\hat{\beta}$ given a specific value of $\alpha$. The final choice of $\hat{\beta}$ would be among $\{\hat{\beta_\alpha}: \alpha \in I_\alpha\}$, whee $I_\alpha$ denotes the set of all $\alpha$'s in consideration.

Let $\xi^{te}$ denote the test set.  Comparing all the 
$$
...
Unsupervised learning is the process of finding patterns in data without any labels(i.e., a dependent variable). It is commonly used for tasks like clustering (e.g., customer segmentation in marketing to group similar customers based on purchasing behavior), dimensionality reduction (e.g., using PCA to compress high-dimensional gene expression data in bioinformatics), and anomaly detection (e.g., identifying fraudulent transactions in financial systems). Unlike supervised learning, where models learn from input-output pairs, unsupervised learning relies on discovering hidden structures within the data itself, making it useful for exploratory data analysis and feature extraction in various domains.


## Self Organizing Maps
**Self organizing maps**, also called **Kohonen maps**, map the input data to a small (often rectangular) space. Similar individuals in the input space are projected into the same neuron in the output space. One can think of this as a one-layer neural network, whose output layer is the map itself.

### Algorithm
The algorithm of SOM is as follows:
1. Set the size of the map and create a grid of neurons with randomly initialized weights. 
  - In SOM, *weights are allocated to each neuron(node of the map), not the input variables. This is distinctive of SOM from supervised learning methods.* Also, the weights can be referred to as **codebooks** in SOMs.
2. Randomly choose an observation from the training data and find the closest node(i.e., **best matching unit(BMU)**).
3. Adjust the weight of the BMU node.
4. Succesively, adjust the weights of the adjacent nodes.
5. Adjust the learning rate and the neighborhood radius so that the next subsequent observation does not drastically affect the model.
6. Repeat steps 2 to 5 for a set number of iterations or until convergence.

### Weight Vectors
In SOM, each node is allocated a weight vector. The vector has the same size to the input vectors(i.e., number of features). For example, if there are 10 features and we set the map into a 8 by 5 rectangular grid, we have 40 weight vectors with the size 8$\times$ 1.

Then, consider the distance between an observation and a node $i$, defined as

$$
d_i = ||x-w_i|| = \sqrt{\sum_{j=1}^n(x_j - w_{ij})^2}
$$

where $n$ is the numbero of observation and $j$ denotes the $j$-th feature. Then, the BMU is defined as

$$
BMU = \arg\min_i (||\mathbf{x}-\mathbf{w}_i||)
$$

The key idea of SOM is to calibrate these weights so that each observation can be classified to the cluster with individuals with the most similar characteristics. The detail of this algorithm is delineated in [A.4.](/#sec-som_details).


Let's begin building our first SOM with the `iris` dataset fro `scikit-learn` module.
```{python}
#| echo: true
#| message: false
#| warning: false
...
### A.2. Recursive Partitioning{#sec-rpart}
...
R(j,s) = \{X|X_j \geq s\}
$$
and $\overline{y}_L, \overline{y}_R$ denote the mean outcome for the training observations in each region.
...
#### Some Remarks for Econ Students like myself
...
$$
which very much resembles the **Herfindahl-Hirschman Index (HHI)** in Industrial Organzation(an index for measuring the concentration of market share). Obviously, different disciplines sometimes rely on similar approaches.
...
### A.4. Self Organizing Map{#sec-som_details}
